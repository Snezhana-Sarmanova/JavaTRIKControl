...
//native void send0(int[] data, int deviceFileDescriptor);
KNIEXPORT KNI_RETURNTYPE_VOID Java_com_trikset_control_I2cCommunicator_send0() {
	...
	KNI_DeclareHandle(arrayObj);
	KNI_GetParameterAsObject(1, arrayObj);
	char* data;
	int size = KNI_GetArrayLength(arrayObj);
	char gaugeBounds[size];
	int i;

	for(i=0; i < size; i++){
		gaugeBounds[i] = (char)KNI_GetIntArrayElement(arrayObj, i);
	}
...
//native int read0(int[] data, int deviceFileDescriptor);
